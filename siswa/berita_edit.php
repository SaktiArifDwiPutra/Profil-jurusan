...
$cur_siswa = fetch_one("SELECT nama FROM siswa WHERE id_siswa = ?", "i", [$u['id_siswa'] ?? 0]);
...
if (empty($berita['created_by']) || intval($berita['created_by']) !== intval($creator_user_id)) {
    flash_set('error','Anda tidak berhak mengedit berita ini.');
    header('Location: /ProfilJurusan/siswa/dashboard.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($judul === '' || $isi === '') {
        ...
        header('Location: /ProfilJurusan/siswa/berita_edit.php?id='.$id);
        exit;
    }
    ...
    $stmt = $conn->prepare("UPDATE berita SET judul=?, isi=?, gambar=? WHERE id_berita=? AND created_by=?");
    $stmt->bind_param('sssii', $judul, $isi, $gambarFilename, $id, $creator_user_id);
    $ok = $stmt->execute();
...
if (gambarInput) {
  ...
  previewWrap.className = 'mt-2 d-none';
  previewWrap.innerHTML = `<label class="form-label small">Preview:</label>
  ...
  gambarInput.parentNode.appendChild(previewWrap);
